...
Server.prototype = {
  ...
  start: function(callback) {
    ...
    if (this.config.shouldBindToHost) {
        winston.info("Using bound hostname:port pair " + this.config.hostname + ":" + this.config.port);
        this.server.listen(this.config.port, this.config.hostname, callback);
    }
    else {
        winston.info("Using bound port only " + this.config.port);
